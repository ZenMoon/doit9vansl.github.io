...
void function (global, factory) {
  ...
  if ((typeof module === "undefined" ? "undefined" : _typeof(module)) === 'object' && _typeof(module.exports) === 'object') {
    module.exports = factory(global, true); // AMD
...
}(typeof window !== 'undefined' ? window : void 0, function (window, noGlobalNS) {
  'use strict';
  ...
  var VERSION = '3.3.0';
  var ROUTINE = [// Initialise the condition with feature-detecting
  // classes (Modernizr-alike), binding onto the root
  // element, possibly `<html>`.
  'initCond', // Address element normalisation
  'renderElem', // Handle Biaodian

  /* 'jinzify', */
  'renderJiya', 'renderHanging', // Address Biaodian correction
  'correctBiaodian', // Address Hanzi and Western script mixed spacing
  'renderHWS', // Address presentational correction to combining ligatures
  'substCombLigaWithPUA' // Address semantic correction to inaccurate characters
  // **Note:** inactivated by default

  /* 'substInaccurateChar', */
  ]; // Define Han

  var Han = function Han(context, condition) {
  ...
  Han.fn = Han.prototype = {
    ...
    routine: ROUTINE,
    init: init,
    setRoutine: function setRoutine(routine) {
    ...
    render: function render(routine) {
      var it = this;
      var routine = Array.isArray(routine) ? routine : this.routine;
      routine.forEach(function (method) {
        if (typeof method === 'string' && typeof it[method] === 'function') {
          it[method]();
        } else if (Array.isArray(method) && typeof it[method[0]] === 'function') {
  ...
  var TYPESET = function () {
    ...
    var rAlph = rLatn + '|' + rGk + '|' + rCy; // For words like `it's`, `Jones’s` or `'99`
    ...
    return {
       ...
       */
      group: {
        biaodian: [new RegExp('((' + rBD + '){2,})', 'g'), new RegExp('(' + rBDLiga + rBDOpen + ')', 'g')],
        punct: null,
        hanzi: new RegExp('(' + rHan + ')+', 'g'),
       ...
       */
      hws: {
        base: [new RegExp('(' + rHan + ')(' + rAlph + '|' + rPtOpen + ')', 'ig'), new RegExp('(' + rAlph + '|' + rPtEnd + ')(' + rHan + ')', 'ig')],
        strict: [new RegExp('(' + rHan + ')' + rWhite + '?(' + rAlph + '|' + rPtOpen + ')', 'ig'), new RegExp('(' + rAlph + '|' + rPtEnd + ')' + rWhite + '?(' + rHan + ')', 'ig')]
      },
      ...
      'display-as': {
        'ja-font-for-hant': [// '夠 够',
        '查 査', '啟 啓', '鄉 鄕', '值 値', '污 汚'],
        'comb-liga-pua': [["a[\u030D\u0358]", "\uDB80\uDC61"], ["e[\u030D\u0358]", "\uDB80\uDC65"], ["i[\u030D\u0358]", "\uDB80\uDC69"], ["o[\u030D\u0358]", "\uDB80\uDC6F"], ["u[\u030D\u0358]", "\uDB80\uDC75"], ["\u31B4[\u030D\u0358]", "\uDB8C\uDDB4"], ["\u31B5[\u030D\u0358]", "\uDB8C\uDDB5"], ["\u31B6[\u030D\u0358]", "\uDB8C\uDDB6"], ["\u31B7[\u030D\u0358]", "\uDB8C\uDDB7"]],
        'comb-liga-vowel': [["a[\u030D\u0358]", "\uDB80\uDC61"], ["e[\u030D\u0358]", "\uDB80\uDC65"], ["i[\u030D\u0358]", "\uDB80\uDC69"], ["o[\u030D\u0358]", "\uDB80\uDC6F"], ["u[\u030D\u0358]", "\uDB80\uDC75"]],
        'comb-liga-zhuyin': [["\u31B4[\u030D\u0358]", "\uDB8C\uDDB4"], ["\u31B5[\u030D\u0358]", "\uDB8C\uDDB5"], ["\u31B6[\u030D\u0358]", "\uDB8C\uDDB6"], ["\u31B7[\u030D\u0358]", "\uDB8C\uDDB7"]]
      },
  ...
  var $ = {
     ...
     * node lists.
     */
    id: function id(selector, $context) {
      return ($context || document).getElementById(selector);
    },
    tag: function tag(selector, $context) {
      return this.makeArray(($context || document).getElementsByTagName(selector));
    },
    ...
    parent: function parent($node, selector) {
     ...
     */
    create: function create(name, clazz) {
      var $elmt = '!' === name ? document.createDocumentFragment() : '' === name ? document.createTextNode(clazz || '') : document.createElement(name);
     ...
     */
    setAttr: function setAttr(target, attr) {
      if (_typeof(attr) !== 'object') return;
      var len = attr.length; // Native `NamedNodeMap``:

      if (_typeof(attr[0]) === 'object' && 'name' in attr[0]) {
        for (var i = 0; i < len; i++) {
     ...
     */
    isIgnorable: function isIgnorable($node) {
     ...
     */
    extend: function extend(target, object) {
      if ((_typeof(target) === 'object' || typeof target === 'function') && _typeof(object) === 'object') {
  ...
  function (Finder) {
    ...
    var Fibre = function Fibre(context, preset) {
    ...
    Fibre.fn = Fibre.prototype = {
      ...
      selector: {},
      preset: 'prose',
      init: function init(context, noPreset) {
        ...
        if (!context) {
          ...
          if (context instanceof Document) this.context = context.body || context;else this.context = context;
      ...
      boundaryFn: function boundaryFn(node) {
      ...
      endFilter: function endFilter(all) {
        if (all) {
      ...
      endAvoid: function endAvoid(all) {
        if (all) {
          this.selector.avoid = [];
        } else {
      ...
      addBoundary: function addBoundary(selector) {
        if (typeof selector === 'string') {
      ...
      setMode: function setMode(portionMode) {
        this.portionMode = portionMode === 'first' ? 'first' : 'retain';
        return this;
      },
      replace: function replace(regexp, newSubStr) {
        var it = this;
        it.finder.push(Finder(it.context, {
          find: regexp,
          replace: newSubStr,
          filterElements: function filterElements(currentNode) {
            return it.filterFn(currentNode);
          },
          forceContext: function forceContext(currentNode) {
      ...
      wrap: function wrap(regexp, strElemName) {
        var it = this;
        it.finder.push(Finder(it.context, {
          find: regexp,
          wrap: strElemName,
      ...
      revert: function revert(level) {
        var max = this.finder.length;
        var level = Number(level) || (level === 0 ? Number(0) : level === 'all' ? max : 1);
        if (typeof max === 'undefined' || max === 0) return this;else if (level > max) level = max;
    ...
    Fibre.fn.filterOut = Fibre.fn.avoid; // Make sure init() inherit from Fibre()
  ...
  function () {
    ...
    function Finder(node, options) {
      ...
      this.node = node;
      this.options = options; // ENable match-preparation method to be passed as option:
    ...
    Finder.prototype = {
      ...
      processMatches: function processMatches() {
        ...
        out: while (true) {
          ...
          if (startPortion && endPortion) {
            curNode = this.replaceMatch(match, startPortion, innerPortions, endPortion); // processMatches has to return the node that replaced the endNode
      ...
      prepareReplacementString: function prepareReplacementString(string, portion, match, matchIndex) {
      ...
      getPortionReplacementNode: function getPortionReplacementNode(portion, match, matchIndex) {
  ...
  function getFuncOrElmt(obj) {
    return typeof obj === 'function' || obj instanceof Element ? obj : undefined;
  }

  function createBDGroup(portion) {
    var clazz = portion.index === 0 && portion.isEnd ? 'biaodian cjk' : 'biaodian cjk portion ' + (portion.index === 0 ? 'is-first' : portion.isEnd ? 'is-end' : 'is-inner');
    var $elmt = $.create('h-char-group', clazz);
    $elmt.innerHTML = portion.text;
    return $elmt;
  }

  function createBDChar(_char) {
    var div = $.create('div');

    var unicode = _char.charCodeAt(0).toString(16);

    div.innerHTML = '<h-char unicode="' + unicode + '" class="biaodian cjk ' + getBDType(_char) + '">' + _char + '</h-char>';
    return div.firstChild;
  }

  function getBDType(_char2) {
    return _char2.match(TYPESET["char"].biaodian.open) ? 'bd-open' : _char2.match(TYPESET["char"].biaodian.close) ? 'bd-close bd-end' : _char2.match(TYPESET["char"].biaodian.end) ? /(?:\u3001|\u3002|\uff0c)/i.test(_char2) ? 'bd-end bd-cop' : 'bd-end' : _char2.match(new RegExp(UNICODE.biaodian.liga)) ? 'bd-liga' : _char2.match(new RegExp(UNICODE.biaodian.middle)) ? 'bd-middle' : '';
  }
  ...
  $.extend(Fibre.fn, {
    ...
    jinzify: function jinzify(selector) {
      return this.filter(selector || null).avoid('h-jinze').replace(TYPESET.jinze.touwei, function (portion, match) {
    ...
    groupify: function groupify(option) {
      var option = $.extend({
        biaodian: false,
        //punct: false,
        hanzi: false,
      ...
      if (option.biaodian) {
        this.replace(TYPESET.group.biaodian[0], createBDGroup).replace(TYPESET.group.biaodian[1], createBDGroup);
      }

      if (option.hanzi || option.cjk) {
        this.wrap(TYPESET.group.hanzi, $.clone($.create('h-char-group', 'hanzi cjk')));
      }

      if (option.western) {
        this.wrap(TYPESET.group.western, $.clone($.create('h-word', 'western')));
      }

      if (option.kana) {
        this.wrap(TYPESET.group.kana, $.clone($.create('h-char-group', 'kana')));
      }

      if (option.eonmun || option.hangul) {
        this.wrap(TYPESET.group.eonmun, $.clone($.create('h-word', 'eonmun hangul')));
      }

      this.endAvoid();
      return this;
    },
    charify: function charify(option) {
      var option = $.extend({
        avoid: true,
        biaodian: false,
        punct: false,
      ...
      if (option.biaodian) {
        this.replace(TYPESET["char"].biaodian.all, getFuncOrElmt(option.biaodian) || function (portion) {
          return createBDChar(portion.text);
        }).replace(TYPESET["char"].biaodian.liga, getFuncOrElmt(option.biaodian) || function (portion) {
          return createBDChar(portion.text);
        });
      }

      if (option.hanzi || option.cjk) {
        this.wrap(TYPESET["char"].hanzi, getFuncOrElmt(option.hanzi || option.cjk) || $.clone($.create('h-char', 'hanzi cjk')));
      }

      if (option.punct) {
        this.wrap(TYPESET["char"].punct.all, getFuncOrElmt(option.punct) || $.clone($.create('h-char', 'punct')));
      }

      if (option.latin) {
        this.wrap(TYPESET["char"].latin, getFuncOrElmt(option.latin) || $.clone($.create('h-char', 'alphabet latin')));
      }

      if (option.ellinika || option.greek) {
        this.wrap(TYPESET["char"].ellinika, getFuncOrElmt(option.ellinika || option.greek) || $.clone($.create('h-char', 'alphabet ellinika greek')));
      }

      if (option.kirillica || option.cyrillic) {
        this.wrap(TYPESET["char"].kirillica, getFuncOrElmt(option.kirillica || option.cyrillic) || $.clone($.create('h-char', 'alphabet kirillica cyrillic')));
      }

      if (option.kana) {
        this.wrap(TYPESET["char"].kana, getFuncOrElmt(option.kana) || $.clone($.create('h-char', 'kana')));
      }

      if (option.eonmun || option.hangul) {
        this.wrap(TYPESET["char"].eonmun, getFuncOrElmt(option.eonmun || option.hangul) || $.clone($.create('h-char', 'eonmun hangul')));
      }
  ...
  void ['setMode', 'wrap', 'replace', 'revert', 'addBoundary', 'removeBoundary', 'avoid', 'endAvoid', 'filter', 'endFilter', 'jinzify', 'groupify', 'charify'].forEach(function (method) {
  ...
  function compareCanvases(treat, control) {
    ...
    try {
      for (var j = 1; j <= 20; j++) {
        for (var i = 1; i <= 50; i++) {
          if (typeof ret === 'undefined' && a.getImageData(i, j, 1, 1).data[3] !== b.getImageData(i, j, 1, 1).data[3]) {
            ret = false;
            break;
  ...
  Locale.support = function () {
    var PREFIX = 'Webkit Moz ms'.split(' '); // Create an element for feature detecting
    // (in `testCSSProp`)
    ...
    function injectElementWithStyle(rule, callback) {
      ...
      ret = callback(container, rule); // Remove the injected scope
    ...
    return {
      ...
      fontface: function () {
        var ret;
        injectElementWithStyle('@font-face { font-family: font; src: url("//"); }', function (node, rule) {
      ...
      ruby: function () {
        ...
        ruby.appendChild(rp);
        ruby.appendChild(rt);
        root.appendChild(ruby); // Browsers that support ruby hide the `<rp>` via `display: none`

        ret = getStyle(rp, 'display') === 'none' || // but in IE, `<rp>` has `display: inline`, so the test needs other conditions:
        getStyle(ruby, 'display') === 'ruby' && getStyle(rt, 'display') === 'ruby-text' ? true : false; // Remove and clean from memory
      ...
      unicoderange: function () {
        var ret;
        injectElementWithStyle('@font-face{font-family:test-for-unicode-range;src:local(Arial),local("Droid Sans")}@font-face{font-family:test-for-unicode-range;src:local("Times New Roman"),local(Times),local("Droid Serif");unicode-range:U+270C}', function () {
          ret = !Locale.detectFont('test-for-unicode-range', // treatment group
  ...
  var SUPPORT_IC = Locale.support['ruby-interchar']; // 1. Simple ruby polyfill;
  ...
  function renderSimpleRuby($ruby) {
    ...
    $.tag('rt', frag.firstChild).forEach(function ($rt) {
      var $rb = $.create('!');
      var airb = [];
      var irb; // Consider the previous nodes the implied
      // ruby base
      ...
      do {
        ...
        airb.push(irb);
      } while (!irb.nodeName.match(/((?:h\-)?r[ubt])/i)); // Create a real `<h-ru>` to append.


      $ru = clazz.contains('zhuyin') ? createZhuyinRu($rb, $rt) : createNormalRu($rb, $rt); // Replace the ruby text with the new `<h-ru>`,
  ...
  function renderInterCharRuby($ruby) {
    ...
    $.tag('rt', frag.firstChild).forEach(function ($rt) {
      var $rb = $.create('!');
      var airb = [];
      var irb, $zhuyin; // Consider the previous nodes the implied
      // ruby base
  ...
  function renderComplexRuby($ruby) {
    ...
    $rb = $ru = $.tag('rb', $cloned);
    maxspan = $rb.length; // First of all, deal with Zhuyin containers
    // individually
    //
    ...
    }($cloned.querySelector('rtc.zhuyin')); // Then, normal annotations other than Zhuyin
    ...
    $.qsa('rtc:not(.zhuyin)', $cloned).forEach(function ($rtc, order) {
      ...
      $ru = ret;
      if (order === 1) $cloned.setAttribute('doubleline', 'true'); // Remove the container once it's useless
  ...
  function simplifyRubyClass(elem) {
    ...
    if (clazz.contains('pinyin')) clazz.add('romanization');else if (clazz.contains('romanization')) clazz.add('annotation');else if (clazz.contains('mps')) clazz.add('zhuyin');else if (clazz.contains('rightangle')) clazz.add('complex');
    return elem;
  }
  /**
  ...
  function createZhuyinRu($rb, $rt) {
    var $rb = $.clone($rb); // Create an element to return

    var $ru = $.create('h-ru');
    $ru.setAttribute('zhuyin', true); // - <h-ru zhuyin>
    // -   <rb><rb/>
    // -   <h-zhuyin>
    // -     <h-yin></h-yin>
    // -     <h-diao></h-diao>
  ...
  $.extend(Locale, {
    ...
    renderRuby: function renderRuby(context, target) {
      var target = target || 'ruby';
      var $target = $.qsa(target, context);
      $.qsa('rtc', context).concat($target).map(simplifyRubyClass);
      $target.forEach(function ($ruby) {
        var clazz = $ruby.classList;
        var $new;
        if (clazz.contains('complex')) $new = renderComplexRuby($ruby);else if (clazz.contains('zhuyin')) $new = SUPPORT_IC ? renderInterCharRuby($ruby) : renderSimpleRuby($ruby); // Finally, replace it
    ...
    renderInterCharRuby: renderInterCharRuby // ### TODO list ###
    //
    // * Debug mode
  ...
  $.extend(Locale, {
    ...
    // ruby -> u, ins -> s, del -> em
    //
    renderElem: function renderElem(context) {
    ...
    renderDecoLine: function renderDecoLine(context, target) {
      var $$target = $.qsa(target || 'u, ins', context);
      var i = $$target.length;

      traverse: while (i--) {
        var $this = $$target[i];
        var $prev = null; // Ignore all `<wbr>` and comments in between,
    ...
    // emphasis marks.
    renderEm: function renderEm(context, target) {
      ...
      $target.forEach(function (elem) {
        ...
        if (Locale.support.textemphasis) {
          $elem.avoid('rt, h-char').charify({
            biaodian: true,
            punct: true
          });
        } else {
          $elem.avoid('rt, h-char, h-char-group').jinzify().groupify({
            western: true
          }).charify({
            hanzi: true,
            biaodian: true,
  ...
  $.extend(Han.support, {
    ...
    fangsong: Han.detectFont('"Han Fangsong"') // 'fangsong-gb': Han.detectFont( '"Han Fangsong GB"' )
  ...
  Han.correctBiaodian = function (context) {
    ...
    finder.avoid('h-char').replace(/([‘“])/g, function (portion) {
    ...
    return Han.support.unicoderange ? finder : finder.charify({
      biaodian: true
    });
  };
  ...
  $.extend(Han.fn, {
    biaodian: null,
    correctBiaodian: function correctBiaodian() {
      this.biaodian = Han.correctBiaodian(this.context);
      return this;
    },
    revertCorrectedBiaodian: function revertCorrectedBiaodian() {
      try {
  ...
  function properlyPlaceHWSBehind($node, text) {
    ...
    if ($.isElmt($node.nextSibling) || sharingSameParent($node, $node.nextSibling)) {
      return text + hws;
    } else {
  ...
  function firstStepLabel(portion, mat) {
    return portion.isEnd && portion.index === 0 ? mat[1] + hws + mat[2] : portion.index === 0 ? properlyPlaceHWSBehind(portion.node, portion.text) : portion.text;
  }
  ...
  function apostrophe(portion) {
    ...
    if ($elmt.nodeName.toLowerCase() === 'h-hws' && (portion.index === 1 || portion.indexInMatch === last$hwsIdx)) {
  ...
  $.extend(Han, {
    renderHWS: function renderHWS(context, strict) {
      ...
      var context = context || document;
      var finder = Han.find(context);
      finder.avoid(AVOID) // Basic situations:
      // - 字a => 字<hws/>a
      // - A字 => A<hws/>字
      .replace(Han.TYPESET.hws[mode][0], firstStepLabel).replace(Han.TYPESET.hws[mode][1], firstStepLabel) // Convert text nodes `<hws/>` into real element nodes:
      .replace(new RegExp('(' + hws + ')+', 'g'), real$hwsElmt) // Deal with:
      // - '<hws/>字<hws/>' => '字'
      // - "<hws/>字<hws/>" => "字"
      .replace(/([\'"])\s(.+?)\s\1/g, apostrophe) // Deal with:
      // - <hws/>“字”<hws/>
      // - <hws/>‘字’<hws/>
      .replace(/\s[‘“]/g, curveQuote).replace(/[’”]\s/g, curveQuote).normalize(); // Return the finder instance for future usage

      return finder;
    }
  });
  $.extend(Han.fn, {
    renderHWS: function renderHWS(strict) {
      Han.renderHWS(this.context, strict);
      return this;
    },
    revertHWS: function revertHWS() {
  ...
  $.extend(Han, {
    ...
    renderHanging: function renderHanging(context) {
      var context = context || document;
      var finder = Han.find(context);
      finder.avoid('textarea, code, kbd, samp, pre').avoid(HANGABLE_AVOID).replace(TYPESET.jinze.hanging, function (portion) {
        if (/^[\x20\t\r\n\f]+$/.test(portion.text)) {
          return '';
        }
        ...
        return !$bd ? $new : function () {
          $bd.classList.add(HANGABLE_CLASS);
          return matches($elmt, 'h-inner, h-inner *') ? biaodian : $new.firstChild;
        }();
      });
      return finder;
    }
  ...
  $.extend(Han.fn, {
    renderHanging: function renderHanging() {
    ...
    revertHanging: function revertHanging() {
      $.qsa('h-char.bd-hangable, h-cs.hangable-outer', this.context).forEach(function ($elmt) {
  ...
  function trimBDClass(clazz) {
    return clazz.replace(/(biaodian|cjk|bd-jiya|bd-consecutive|bd-hangable)/gi, '').trim();
  }
  ...
  function insertJiyaCS($jinze) {
    if (matches($jinze, '.tou, .touwei') && !matches($jinze.previousSibling, 'h-cs.jiya-outer')) {
      $jinze.insertAdjacentHTML('beforebegin', JIYA_CS_HTML);
    }

    if (matches($jinze, '.wei, .touwei') && !matches($jinze.nextSibling, 'h-cs.jiya-outer')) {
  ...
  Han.renderJiya = function (context) {
    var context = context || document;
    var finder = Han.find(context);
    finder.avoid('textarea, code, kbd, samp, pre, h-cs').avoid(JIYA_AVOID).charify({
      avoid: false,
      biaodian: charifyBiaodian
    }) // End avoiding `JIYA_AVOID`:
    .endAvoid().avoid('textarea, code, kbd, samp, pre, h-cs').replace(TYPESET.group.biaodian[0], locateConsecutiveBD).replace(TYPESET.group.biaodian[1], locateConsecutiveBD);
    return finder;
  };

  $.extend(Han.fn, {
    renderJiya: function renderJiya() {
      Han.renderJiya(this.context);
      return this;
    },
    revertJiya: function revertJiya() {
      $.qsa('h-char.bd-jiya, h-cs.jiya-outer', this.context).forEach(function ($elmt) {
        var classList = $elmt.classList;
  ...
  function createSubstFactory(regexToSubst) {
    return function (context) {
      ...
      regexToSubst.forEach(function (pattern) {
        finder.replace(new RegExp(pattern[0], 'ig'), function (portion, match) {
          var ret = $.clone(charCombLiga); // Put the original content in an inner container
  ...
  $.extend(Han, {
    ...
    isCombLigaNormal: isVowelICombLigaNormal()(),
    // ### Deprecated
    ...
    substInaccurateChar: function substInaccurateChar(context) {
      var context = context || document;
      var finder = Han.find(context);
  ...
  $.extend(Han.fn, {
    ...
    substVowelCombLiga: function substVowelCombLiga() {
      this['comb-liga-vowel'] = Han.substVowelCombLiga(this.context);
      return this;
    },
    substVowelICombLiga: function substVowelICombLiga() {
      this['comb-liga-vowel-i'] = Han.substVowelICombLiga(this.context);
      return this;
    },
    substZhuyinCombLiga: function substZhuyinCombLiga() {
      this['comb-liga-zhuyin'] = Han.substZhuyinCombLiga(this.context);
      return this;
    },
    substCombLigaWithPUA: function substCombLigaWithPUA() {
    ...
    revertVowelCombLiga: function revertVowelCombLiga() {
      try {
    ...
    revertVowelICombLiga: function revertVowelICombLiga() {
      try {
    ...
    revertZhuyinCombLiga: function revertZhuyinCombLiga() {
      try {
    ...
    revertCombLigaWithPUA: function revertCombLigaWithPUA() {
      try {
        this['comb-liga-vowel'].revert('all');
        this['comb-liga-vowel-i'].revert('all');
        this['comb-liga-zhuyin'].revert('all');
    ...
    substInaccurateChar: function substInaccurateChar() {
      this['inaccurate-char'] = Han.substInaccurateChar(this.context);
      return this;
    },
    revertInaccurateChar: function revertInaccurateChar() {
      try {
  ...
  window.addEventListener('DOMContentLoaded', function () {
    var initContext; // Use the shortcut under the default situation

    if (root.classList.contains('han-init')) {
      Han.init(); // Consider ‘a configured context’ the special
